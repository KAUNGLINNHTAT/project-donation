    ...
    <div class="container">
        <div class="container mx-auto px-4 sm:px-8 py-4 sm:py-2">
            <div class="flex justify-between">
                <div><span class="font-sans sm:text-2xl md:text-3xl items-center">Donation List Edit</span></div>
        ...
        <section class="bg-gray-100 mx-auto bg-opacity-50 px-4 py-4 sm:py-2 sm:px-8">
            <form class="container mx-auto shadow-md md:w-full" @submit.prevent="updateDonationList">
                <div class="space-y-2 bg-white rounded-lg">
                    ...
                    <div class="w:full px-4 pb-4 mx-auto text-gray-500 text-center">
                        <button type="submit" class="py-2 px-4 bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 focus:ring-offset-blue-200 text-white w-1/3 transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg"> Update Donation List </button>
                    </div>
                </div>
            </form>
        </section>
        <!-- Donation List Form End --> 
...
export default {
    ...
    created() {
        axios
            .get("../../../api/donationlist/" + this.$route.params.id + "/edit")
            .then(response => {
                this.donationlist = response.data.data
                this.publishedRadio(response.data.data.published_flg);
            })
    ...
    methods: {
        ...
        updateDonationList: function() {
            axios
            .put("../../../api/donationlist/"+ this.$route.params.id, this.donationlist)
            .then(result => {
                this.$refs.successModal.toggleModal();
                this.$refs.successModal.themodaltitle = "Donation List Update";
                console.log(result);
            })
